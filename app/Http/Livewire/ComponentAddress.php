...
class ComponentAddress extends Component
{
    ...
    {
        $queryPhone = Address::query();
        if ($this->search != null) {
            $this->updatingSearch();
            $queryPhone = $queryPhone->where('ubication', 'like', '%' . $this->search . '%');
        }
        $addresses = $queryPhone->where('addressable_id', $this->patient->id)->orderBy('id', 'DESC')->paginate(2);
